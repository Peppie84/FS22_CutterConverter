...
local function preload(mission)
    ...
    for cutterConverterName, cutterConverterDesc in pairs(cutterConverterData) do
        ...
        local converterIndex      = g_fruitTypeManager:addFruitTypeConverter(cutterConverterName, true)
        g_fruitTypeManager:addFruitTypeConversion(converterIndex, fromFruitTypeDesc.index, toFruitFillTypeDesc.index, conversionFactor, windrowConversionFactor)
    end
end
